...
- **US-3.1: Morning and Evening Athkar Checklists** ✅
    - _As a_ user, _I want_ to open a checklist for my Morning or Evening Athkar and tap a counter button for each specific supplication, _so that_ I can ensure I read them the correct number of times (e.g., 3 times, 100 times). Cards are shown inline on the Athkar screen — no intermediate sheet required.
...
- **US-5.1: The Home Dashboard Summary** ✅
    - _As a_ user, _I want_ my Home screen to show my "Next Prayer" prominently, Athkar done/pending status, Quran progress ring, Tasbeeh daily progress, Qibla shortcut, and a summary of today's habits, _so that_ I get an instant snapshot of my spiritual day without switching tabs. Tapping any card navigates directly to the corresponding feature screen.
...
- **US-7.4: App Language (English/Arabic)** ✅
    - _As a_ user, _I want_ to switch between English and Arabic, _so that_ the app UI displays in my preferred language. Arabic enables RTL layout on both Android and iOS.
- **US-7.5: Notification Preferences** ✅
    - _As a_ user, _I want_ to enable or disable daily Athkar reminder notifications (morning and evening), _so that_ I receive reminders at my preferred times without needing to open the Athkar screen. Accessible from both Settings and the Athkar screen bell icon.

## Epic 8: Theming & Personalization
- **US-8.1: Dynamic Theming** ✅ (Android only)
...
## Epic 9: Native iOS Experience ✅ Implemented (spec 013)
- **US-9.1: Fully Native iOS UI** ✅
    - _As an_ iOS user, _I want_ all screens to be native SwiftUI — smooth system animations, native swipe-back, native tab bar, native sheets — _so that_ the app feels completely at home on my iPhone.
- **US-9.2: Feature Parity with Android** ✅
    - _As an_ iOS user, _I want_ all 8 feature screens (Home, Prayer, Quran, Athkar, Tasbeeh, Habits, Settings, Qibla) to display real data from the Kotlin domain layer and persist all actions through Room, _so that_ I get the same features as Android.
- **US-9.3: Native iOS Navigation** ✅
    - _As an_ iOS user, _I want_ tab switching, swipe-back from push destinations, and sheet presentation to all use native SwiftUI animation with no jank, _so that_ navigation feels fluid and familiar.
- **US-9.4: Qibla Compass with Native Sensors** ✅
    - _As an_ iOS user, _I want_ the Qibla compass to use `CLLocationManager` heading updates directly, _so that_ compass rotation is smooth and accurate at native speed.
- **US-9.5: Arabic RTL Layout on iOS** ✅
    - _As an_ Arabic-speaking user on iOS, _I want_ the entire app to mirror to RTL when Arabic is selected, _so that_ the layout direction matches my natural reading direction.
- **US-9.6: Inline Athkar Tap-to-Count** ✅
    - _As an_ iOS user reading my Athkar, _I want_ the dhikr cards to appear directly on the Athkar screen with live counters, _so that_ I can count without navigating into a separate sheet.
